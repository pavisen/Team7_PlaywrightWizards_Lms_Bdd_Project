...
export class ProgramPage {
  constructor(page) {
    ...
    this.addNewProgram_btn = page.getByRole("menuitem", {
  ...
  async enterProgramDetails(ProgramName, Description) {
    ...
    const DescriptionIn = this.page.getByRole("textbox", {
      name: "Description *",
    });
  ...
  async verifyMandatoryField(field) {
    const mandatoryField = this.page
      .locator("label")
  ...
  editProgramName() {
    return this.page
      ...
      .filter({ name: "Name *" })
      .first()
      .fill("Updated Program Name");
  }
  async verifyProgramName(value) {
    await expect(
      await this.page.getByRole("row", { name: `${value}` }),
    ).toBeVisible();
  }
  editDescription() {
    return this.page
  ...
  async verifyDescription(prog, desc) {
    await expect(
      await this.page.getByRole("row", { name: `${prog}` }),
    ).toBeVisible();
  }
  async changeStatus() {
    return await this.page
      .locator("p-radiobutton")
      .filter({ "ng-reflect-value": "Inactive" })
      .first()
      .click();
  }
  async verifyStatus() {
