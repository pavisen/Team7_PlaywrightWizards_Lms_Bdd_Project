...
class ClassPage {
  constructor(page) {
    ...
    this.staffName = page
      ...
      .filter({ visible: true });
    this.activeStatus = page
      .locator(".p-radiobutton-box")
      .filter({ visible: true })
      .first();
    this.inactiveStatus = page
      .locator(
  ...
  async enterClassDetails(
    ...
    if (classDate) {
      ...
      try {
        ...
        console.error(
          "Timeout: No. of Classes field was not populated within 5 seconds.",
          error,
        );
      }
    }
  ...
  async getNgReflectModelValue() {
    try {
      ...
      const isValuePresent = await this.page
        .waitForFunction(
          (el) => el && el.value.trim() !== "",
  ...
  async verifyWeekendsAndPastDatesDisabled() {
    ...
    console.log(
      `Found ${disabledCount} disabled dates (weekends & past dates).`,
    );
  ...
  async getAllErrorMessages(validateColor = false) {
    const errorMessages = [];
    const errorMessageElements = await this.errorMessages
      .filter({ visible: true })
      .all();
    ...
    for (let i = 0; i < errorMessageElements.length; i++) {
      ...
      if (validateColor) {
        ...
        const expectedColors = [
          ...
          "rgb(220, 53, 69)",
          "rgb(217, 83, 79)",
        ]; // Different possible red shades

        if (!expectedColors.includes(color)) {
          throw new Error(
            `Error message color is incorrect for message: "${message}". Expected one of ${expectedColors}, got: ${color}`,
          );
        }
      }
  ...
  async isBatchNameFieldDisabled() {
    ...
    const disabledAttribute = await this.batchNameField
      .locator("input")
      .first()
      .getAttribute("disabled");

    // Check if the parent div has 'p-disabled' class
    const hasDisabledClass = await this.batchNameField
      .locator("..")
  ...
  async isClassTopicFieldDisabled() {
    ...
    const disabledAttribute =
      await this.classTopicField.getAttribute("disabled");

    // Check if any ancestor has 'p-disabled' class
    const hasDisabledClass =
      (await this.classTopicField
        .locator('xpath=ancestor::*[contains(@class, "p-disabled")]')
        .count()) > 0;
  ...
  async editEnterClassDetails(
    ...
    if (classDate) {
      ...
      await this.classDate.evaluate((el) =>
