...
class BatchPage {
  constructor(page) {
    ...
    this.batchStatusIn = page.locator(
      "div:nth-child(3) > #batchStatus > .p-radiobutton > .p-radiobutton-box",
    );
    this.noOfClasses = page.getByRole("spinbutton", {
    ...
    this.programDroparrow = this.page.locator(
      "//span[@class='p-dropdown-trigger-icon ng-tns-c171-9 pi pi-chevron-down']",
    );
  ...
  async updateBatchDetails(description, numberOfClasses) {
    ...
    await this.noOfClasses.fill(numberOfClasses.toString());
    console.log(
      `Batch details updated: Description - ${description}, No. of Classes - ${numberOfClasses}, Status `,
    );
  }
  ...
  async updateInvalidBatchDetails(description, numberOfClasses) {
    ...
    await this.noOfClasses.fill(numberOfClasses.toString());
    console.log(
      `Batch invalid details update: Description - ${description}, No. of Classes - ${numberOfClasses}, Status `,
    );
  }

  async clickEditFirstRow() {
    const firstRowEditButton = this.page
      .locator("span.p-button-icon.pi.pi-pencil")
      .first();
  ...
  async selectProgramFromDropdown() {
    ...
    const dropdownvalue = this.page.locator(
      "//input[@placeholder='Select a Program name']",
  ...
  async getAllErrorMessages() {
    ...
    for (let i = 0; i < errorMessageElements.length; i++) {
      ...
      const color = await errorMessageElement.evaluate(
        (el) => getComputedStyle(el).color,
      );
      const expectedColor = "rgb(255, 0, 0)"; // Red color in RGB
      if (color !== expectedColor) {
        throw new Error(
          `Error message color is not red for message: "${message}". Expected color: ${expectedColor}, got: ${color}`,
        );
      }
  ...
  async verifyProgramNameFieldIsDisabled() {
    const disabledAttribute = await this.programNameField
      .locator("input")
      .first()
      .getAttribute("disabled");
    const hasDisabledClass = await this.programNameField
      .locator("..")
  ...
  async verifyBatchNameFieldIsDisabled() {
    ...
    const hasDisabledClass = await batchNameField1
      .locator("xpath=..")
      .evaluate((el) => el.classList.contains("p-disabled"));
