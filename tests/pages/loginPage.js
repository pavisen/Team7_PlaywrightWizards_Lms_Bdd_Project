...
class LoginPage {
  ...
  async getUserFieldPlaceholderColor() {
    return await this.usernameFieldLabel.evaluate(
      (el) => getComputedStyle(el).color,
    );
  }

  async getPasswordFieldPlaceholderColor() {
    return await this.passwordFieldLabel.evaluate(
      (el) => getComputedStyle(el).color,
    );
  }
  ...
  async verifyTextFields() {
    expect(await this.usernameInput).toHaveAttribute(
      "ng-reflect-placeholder",
      "User",
    );
    expect(await this.passwordInput).toHaveAttribute(
      "ng-reflect-placeholder",
      "Password",
    );
  }
  ...
  async ExtractTextFromImage() {
    try {
      ...
      await this.Home_image.screenshot({
        path: "homePage.png",
        timeout: 10000,
      });
