...
When(
  ...
  async ({ classPage, commonFunctions, testData, page }) => {
    ...
    const classTopic =
      getTestData(sheetName, "validAll", "classTopic") + newclassTopic;
...
When(
  ...
  async ({ classPage, commonFunctions, testData }) => {
    ...
    console.log(newclassTopic);
    const classTopic =
      getTestData(sheetName, "onlyMandatory", "classTopic") + newclassTopic;
...
Then(
  "Admin gets message Class added Successfully",
...
When(
  "Admin selects class date in date picker",
  async ({ classPage, commonFunctions }) => {
    const classDate =
      getTestData(sheetName, "onlyMandatory", "classDate") || "";
    const staffName =
      getTestData(sheetName, "onlyMandatory", "staffName") || "";
    await classPage.enterClassDetails(
      "",
      "",
      "",
...
Then(
  "Admin should see no of class value is added automatically",
  async ({ classPage }) => {
...
Then(
  "Admin should see weekends dates are disabled to select",
  async ({ classPage }) => {
    const areDatesDisabled =
      await classPage.verifyWeekendsAndPastDatesDisabled();
...
When(
  "Admin skips to add value in mandatory field and enter only the optional field",
  async ({ classPage, commonFunctions }) => {
    const classDescription = getTestData(
      sheetName,
      "onlyOptional",
...
Then(
  "Admin should see error message below the test field and the field will be highlighted in red color",
  async ({ classPage }) => {
...
When(
  "Admin enters invalid data in all of the  fields in the form and clicks on save button",
  async ({ classPage, commonFunctions }) => {
...
Then(
  "Admin gets error message and class is not created",
  async ({ classPage }) => {
    await classPage.page.pause();
...
When(
  "Admin clicks on save button without entering data",
  async ({ classPage }) => {
    await classPage.clickSave();
  },
);
...
Then(
  "class won't be created and Admin gets error message",
  async ({ classPage }) => {
    const errorMessages = await classPage.getAllErrorMessages();
    const expectedErrors = [
      "Batch Name is required.",
...
When(
  "Admin clicks Cancel button on Admin Details form",
  async ({ classPage, commonFunctions }) => {
...
Then(
  "Class Details popup window should be closed without saving",
...
When(
  "Admin clicks Close Icon on Admin Details form",
  async ({ classPage, commonFunctions }) => {
    const batchName = getTestData(sheetName, "onlyMandatory", "batchName");
    const classTopic = getTestData(sheetName, "onlyMandatory", "classTopic");
    const classDate = getTestData(sheetName, "onlyMandatory", "classDate");
