...
When(
  "Admin Clicks on the program menu from the header",
...
Then(
  "Admin should be in the {string} Page for managing program",
...
Then(
  "Admin should see the {string} title in the Program Page",
...
Then(
  "Admin should see the page names as in order Home Program Batch Class",
  async ({ programPage }) => {
    await programPage.click_program();
    const actual_modules = await programPage.modulenames();
    const expected_modulenames = [
      "Home",
      "Program",
      "Batch",
      "Class",
...
Then(
  ...
  async ({ commonFunctions }) => {
    ...
    const headerCells = (
      await commonFunctions.tableHeader.allTextContents()
...
Then(
  "Admin should see Search bar with text as {string}",
  async ({ commonFunctions }, arg) => {
    await commonFunctions.clickMenu("program");
    await commonFunctions.clickAnywhere();
    await expect(commonFunctions.searchBar).toBeVisible();
    const actualText =
      await commonFunctions.searchBarByPlaceholder.getAttribute("placeholder");
    expect(actualText.trim()).toBe(arg);
  },
);

Then(
  "Admin should see checkbox default state as unchecked beside Program Name column as header",
  async ({ commonFunctions }) => {
    await commonFunctions.clickMenu("program");
    await commonFunctions.clickAnywhere();
...
Then(
  "Admin should see check box default state as unchecked on the left side in all rows against program name",
  async ({ commonFunctions }) => {
    await commonFunctions.clickMenu("program");
    await commonFunctions.clickAnywhere();
    await expect(commonFunctions.checkboxHeaderTableBatch).toBeVisible();
...
Then(
  "Admin should see the sort arrow icon beside to each column header except Edit and Delete",
  async ({ commonFunctions }) => {
    await commonFunctions.clickMenu("program");
    await commonFunctions.clickAnywhere();
    await commonFunctions.isSortIconVisible();
  },
);
...
Then(
  ...
  async ({ commonFunctions }) => {
    ...
    await commonFunctions.deleteButtoneachRowBatch
      .first()
      .waitFor({ state: "visible" });
  },
);
...
Then(
  "Admin should see the text as {string} in program",
  async ({ commonFunctions }, expectedText) => {
    await commonFunctions.clickMenu("program");
    await commonFunctions.clickAnywhere();
