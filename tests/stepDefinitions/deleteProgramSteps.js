...
Given(
  ...
  async function ({ loggedInPage, commonFunctions }) {
...
When(
  "Admin clicks on delete button for a program",
...
Then(
  "Admin can see Successful Program Deleted message",
  async ({ programPage }) => {
    const programdeleteMessage =
      await programPage.programdeleteMessage.textContent();
...
Then(
  "Admin can see Confirmation form disappears",
  async ({ commonFunctions }) => {
...
Then(
  "Admin can see Confirm Deletion form disappear",
  async ({ commonFunctions }) => {
