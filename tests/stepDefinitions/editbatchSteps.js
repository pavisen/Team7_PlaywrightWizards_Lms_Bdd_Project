 ...
 const sheetName = 'batch';
  ...
  When('Admin clicks the edit icon	in Batch Page', async ({commonFunctions,batchPage}) => {
  ...
  Then('Admin should see the Batch details pop up window', async ({commonFunctions,batchPage}) => {
 ...
 When('Admin enters the valid data in edit Batch pop up and click save button', async ({batchPage,commonFunctions}) => {
  ...
  await batchPage.clickEdit(storedbatchNameForBatch);
   ...
   // Get the data from the excel file
       ...
       const NoOfClasses = getTestData(sheetName, 'editvaliddata', 'NumberOfClasses');
   ...
   When('Admin clicks the edit icon for program name', async ({batchPage,commonFunctions}) => {
    await batchPage.clickEditFirstRow();

  });
  
  When('Admin clicks the edit icon for batch name', async ({batchPage,commonFunctions}) => {
    await batchPage.clickEditFirstRow();

  });
  
   When('Admin edits the valid data in edit Batch pop up and click cancel button', async ({batchPage,commonFunctions}) => {
    await batchPage.clickEditFirstRow();
     // Get the data from the excel file
         const Description = getTestData(sheetName, 'editvaliddatacancel', 'Description');
         const NoOfClasses = getTestData(sheetName, 'editvaliddatacancel', 'NumberOfClasses');
         console.log(`Description: ${Description}`);
         console.log(`NoOfClasses: ${NoOfClasses}`);
         await batchPage.updateBatchDetails(Description, NoOfClasses); 
  ...
  Then('Admin should see Program name value field is disabled for editing', async ({batchPage}) => {
  ...
  When('Admin Updates any fields with invalid data and click save button', async ({batchPage,commonFunctions}) => {
    await batchPage.clickEditFirstRow();
    // Get the data from the excel file
    const Description = getTestData(sheetName, 'editinvaliddata', 'Description');
    const NoOfClasses = getTestData(sheetName, 'editinvaliddata', 'NumberOfClasses');
    console.log(`Description: ${Description}`);
    console.log(`NoOfClasses: ${NoOfClasses}`);
...
//console.log("Error messages successfully validated.");
//   console.log("Expected error messages:", expectedErrors);
...
//  }
  ...
  Then('Admin should get a {string} for editing the batch', async ({batchPage},arg) => {
  ...
  Then('Admin can see the batch details popup closes without editing the batch', async ({batchPage,commonFunctions}) => {
