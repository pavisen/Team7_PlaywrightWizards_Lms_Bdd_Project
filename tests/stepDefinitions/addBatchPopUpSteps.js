...
Given('Admin is on the Batch Details Pop Up WIndow', async ({commonFunctions}) => {
  ...
  Then('The pop up should include the fields Batch Name,Number of classes and Description as text box,Program Name as dropdown Status as radio button', async ({batchPage}) => {
  ...
  Then('Admin should get error message below the text box of respective field', async ({batchPage}) => {
    ...
    expect(errorMessageText.trim()).toContain('This field accept only numbers and max 5 count.');  });
