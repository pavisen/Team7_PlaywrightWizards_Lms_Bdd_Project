...
When(
  "Admin clicks Next page link on the program table",
...
Then(
  "Admin should see the Pagination has {string} active link",
  async ({ commonFunctions }, next) => {
    await commonFunctions.verifyNextPageEnabled();
  },
);

When(
  "Admin clicks Last page link in program table",
  async ({ commonFunctions }) => {
    await commonFunctions.arePaginationButtonsVisible();
...
When(
  "Admin is on the Last page in program table",
  async ({ commonFunctions }) => {
    await commonFunctions.arePaginationButtonsVisible();
    await commonFunctions.goToLastPage();
  },
...
Then(
  "Admin should see the last page record on the program table with Next page link are disabled",
  async ({ commonFunctions }) => {
...
Given(
  "Admin is on last page of Program module table",
  async ({ commonFunctions }) => {
    await commonFunctions.clickMenu("program");
    await commonFunctions.clickAnywhere();
    await commonFunctions.arePaginationButtonsVisible();
...
Then(
  "Admin should see the previous page record on the table with pagination has previous page link",
  async ({ commonFunctions }) => {
    await commonFunctions.verifyNextPageEnabled();
  },
);
...
Then(
  "Admin should see the very first page record on the program table with Previous page link are disabled",
  async ({ commonFunctions }) => {
