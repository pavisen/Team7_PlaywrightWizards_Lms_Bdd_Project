...
Given(
  "Admin is on the Manage Class page",
...
When(
  "Admin clicks on the edit icon",
  async ({ commonFunctions, classPage, testData }) => {
...
Then(
  "Admin should see class topic field is disabled",
  async ({ classPage }) => {
...
Given(
  "Admin is on the Edit Class Popup window",
  async ({ commonFunctions, classPage }) => {
    await commonFunctions.clickMenu("class");
    await commonFunctions.clickAnywhere();
  },
);
...
When(
  "Update the fields with valid data and click save",
...
Then(
  "Admin gets message class details updated Successfully",
  async ({ classPage }) => {
...
When(
  "Update the fields with invalid values and click save",
  async ({ classPage, commonFunctions, testData, page }) => {
    const disabledName = getTestData(sheetName, "disabledCheck", "classTopic");
    await commonFunctions.search(disabledName);
    await classPage.clickEdit(`Java Batch 01 ${disabledName}`);
...
When(
  "Update the mandatory fields with valid values and click save",
  async ({ classPage, commonFunctions, testData, page }) => {
    await commonFunctions.search("Java Batch 01");
    await classPage.clickEdit("Java Batch 01");
    const classDescription = getTestData(
...
When(
  "Update the optional fields with valid values and click save",
  async ({ classPage, commonFunctions, testData, page }) => {
    await commonFunctions.search("Java Batch 01");
    await classPage.clickEdit("Java Batch 01");
...
When(
  "Admin clicks Cancel button on edit popup",
...
Then(
  "Admin can see the class details popup disappears and can see nothing changed for particular Class",
