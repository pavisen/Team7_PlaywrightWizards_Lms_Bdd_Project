...
Then(
  "Admin should see a alert open with heading {string} along with  <YES> and <NO> button for deletion",
...
Then(
  "Admin gets a message {string} alert and do not see that Class in the data table",
  async ({ commonFunctions }, arg) => {
    const successMessage = await commonFunctions
      .getDeleteMessage("Class")
      .textContent();
...
Then(
  "Admin can see the deletion alert disappears without deleting",
...
Then(
  "Admin can see the deletion alert disappears without any changes",
  async ({ commonFunctions }) => {
...
When(
  "Admin clicks single checkbox in the data table and Admin clicks <YES> button on the alert",
  async ({ commonFunctions }) => {
    await commonFunctions.clickDeleteRow(1);
    await commonFunctions.clickConfirmYesDelete();
  },
...
Then(
  "Admin should land on Manage class page and can see the selected class is deleted from the data table",
  async ({ commonFunctions }) => {
    const successMessage = await commonFunctions
      .getDeleteMessage("Class")
      .textContent();
...
When(
  "Admin clicks single checkbox in the data table and Admin clicks <NO> button on the alert",
  async ({ commonFunctions }) => {
    await commonFunctions.clickDeleteRow(1);
    await commonFunctions.clickConfirmNoDelete();
  },
...
Then(
  "Admin should land on Manage class page and can see the selected class is not deleted from the data table",
  async ({ commonFunctions }) => {
    const confirmDialog = await commonFunctions.confirmDialog;
    await expect(confirmDialog).not.toBeVisible();
  },
...
When(
  "Admin clicks any checkbox in the data table and Admin clicks <YES> button on the alert for multiple",
  async ({ commonFunctions }) => {
...
Then(
  "Admin should land on Manage class page and can see the selected class are deleted from the data table",
  async ({ commonFunctions }) => {
    const successMessage = await commonFunctions
      .getDeletedMessage("Class")
      .textContent();
...
When(
  "Admin clicks any checkbox in the data table and Admin clicks <NO> button on the alert for multiple",
  async ({ commonFunctions }) => {
    await commonFunctions.clickDeleteRow(2);
    await commonFunctions.clickConfirmNoDelete();
  },
