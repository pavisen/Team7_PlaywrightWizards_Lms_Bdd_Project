...
When(
  "Admin Clicks on the Add Batch button and fill the required fields",
  async function ({ batchPage, commonFunctions, testData }) {
    ...
    const BatchName =
      getTestData(sheetName, "valid", "BatchName") + getRandomNumberString(1);
    ...
    await batchPage.enterBatchDetails(
      ProgramName,
      BatchName,
      Description,
      NoOfClasses,
...
When(
  "Admin Clicks on the Add Batch button and fill the mandatory fields",
  async ({ batchPage, commonFunctions, testData }) => {
    await commonFunctions.clickMenu("batch");
    await commonFunctions.clickSubMenu("batch");
    ...
    const BatchName =
      getTestData(sheetName, "mandatory", "BatchName") +
...
When(
  "Admin leaves blank one of the mandatory fields",
  async function ({ batchPage }) {
  
    // Get the data from the excel file
    const ProgramName = getTestData(
      sheetName,
      "missingmandatory",
      "ProgramName",
    );
...
Then(
  "Admin should get a error message on the respective mandatory field",
  async function ({ batchPage }) {
...
When(
  "Admin enters the valid data and click save button",
  async function ({ batchPage }) {
...
When(
  "Admin enters the valid data and click cancel button",
  async function ({ batchPage, commonFunctions }) {
...
Then(
  "Admin can see the batch details popup closes without creating any batch",
...
When(
  "Admin clicks on the close icon",
  async function ({ batchPage, commonFunctions }) {
