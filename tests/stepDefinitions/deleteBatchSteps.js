...
Then('Admin should see the confirm alert box with yes and no button', async ({commonFunctions}) => {
...
Given('Admin is on the batch confirm popup page', async ({commonFunctions}) => {
  await commonFunctions.clickMenu('batch');
  await commonFunctions.clickAnywhere();
  await commonFunctions.clickDeleteRow(1);
});

When('Admin clicks on the delete icon and click yes button', async ({commonFunctions}) => {
  await commonFunctions.clickConfirmYesDelete();
});

Then('Admin should see the successful message and the batch should be deleted', async ({commonFunctions}) => {
...
When('Admin clicks on the delete icon and click no button', async ({commonFunctions}) => {
  await commonFunctions.clickConfirmNoDelete();
});

Then('Admin should see the alert box closed and the batch is not deleted', async ({commonFunctions}) => {
...
When ('Admin clicks on the close icon on delete', async ({commonFunctions})=>{
 
  await commonFunctions.clickCloseDelete();
});

Then('Admin should see the alert box closed', async ({batchPage,commonFunctions}) => {
